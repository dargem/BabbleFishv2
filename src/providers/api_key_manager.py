...
class APIKeyManager:
    ...
    async def get_available_key(self) -> str:
        ...
        async with self._lock:
            ...
            available_keys = [
                stats for stats in self._keys.values()
            ...
            logger.debug(f"Selected API key ending in ...{selected_stats.key[-4:]} "
    ...
    async def mark_key_error(self, api_key: str, error: Exception) -> None:
        ...
        if api_key in self._keys:
            ...
            if stats.error_count >= 3:
                ...
                logger.warning(f"API key ending in ...{api_key[-4:]} marked as unhealthy "
    ...
    async def is_key_healthy(self, api_key: str) -> bool:
        ...
        if (stats.last_health_check is None or 
            datetime.now() - stats.last_health_check > timedelta(minutes=10)):
    ...
    async def _health_check_key(self, api_key: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Health check failed for key ending in ...{api_key[-4:]}: {e}")
    
    async def refresh_unhealthy_keys(self) -> None:
        """Attempt to refresh all unhealthy keys by health checking them."""
        unhealthy_keys = [
            key for key, stats in self._keys.items() 
