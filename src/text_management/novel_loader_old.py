...
class NovelTextLoader:
    ...
    def load_from_fanqie_url(
        ...
        return self.load_from_fanqie_id(
            novel_id, novel, chapter_limit, force_redownload
        )
    ...
    def load_from_fanqie_id(
        ...
        if not result["success"]:
            raise RuntimeError(
                f"Failed to download novel: {result.get('error', 'Unknown error')}"
            )
    ...
    def search_and_load_fanqie(
        ...
        if novel_index >= len(novels_data):
            raise RuntimeError(
                f"Novel index {novel_index} out of range (found {len(novels_data)} novels)"
            )
    ...
    def load_from_directory(
        ...
        logger.info(
            "Loaded %d chapters from directory: %s", len(chapters), directory_path
        )
        return novel
...
def search_and_load_fanqie_novel(
    ...
    loader = NovelTextLoader(download_dir)
    return loader.search_and_load_fanqie(
        query, chapter_limit=chapter_limit, novel_index=novel_index
    )
