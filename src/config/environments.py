...
class DevelopmentConfig(ConfigLoader):
    ...
    def _get_api_keys(self) -> list:
        ...
        if not keys:
            raise ValueError("No Google API keys found. Set GOOGLE_API_KEY or GOOGLE_API_KEY_1, GOOGLE_API_KEY_2, etc.")
...
class ProductionConfig(ConfigLoader):
    ...
    def load(self) -> AppConfig:
        ...
        return AppConfig(
            ...
            llm=LLMConfig(
                ...
                temperature=float(os.getenv("LLM_TEMPERATURE", "0.3")),  # More deterministic
                api_keys=api_keys,
            ...
            workflow=WorkflowConfig(
                ...
                enable_mermaid_graphs=bool(os.getenv("WORKFLOW_ENABLE_MERMAID", "false").lower() == "true"),
                chapter_processing_batch_size=int(os.getenv("WORKFLOW_BATCH_SIZE", "10"))
            )
        )
    ...
    def _get_api_keys(self) -> list:
        ...
        # Look for numbered API keys
        for i in range(1, 20):  # Support more keys in production
            key = os.getenv(f"GOOGLE_API_KEY_{i}")
            if key:
                keys.append(key)
...
class ConfigFactory:
    ...
    def create_config(env: str = None) -> AppConfig:
        ...
        if env not in loaders:
            raise ValueError(f"Unknown environment: {env}. Must be one of: {list(loaders.keys())}")
