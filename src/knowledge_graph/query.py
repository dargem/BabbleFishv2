...
class KnowledgeGraphQuery:
    ...
    def find_connected_entities(self, entity_name: str, max_depth: int = 2) -> List[Dict[str, Any]]:
        """
        ...
        """
        with self.graph_manager.driver.session() as session:
            return session.execute_read(self._find_connected_entities_tx, entity_name, max_depth)
    
    def find_entity_mentions_across_chapters(self, entity_name: str) -> List[Dict[str, Any]]:
        """
    ...
    def find_entities_by_relationship(self, predicate: str, chapter_idx: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        with self.graph_manager.driver.session() as session:
            return session.execute_read(self._find_entities_by_relationship_tx, predicate, chapter_idx)
    
    def get_character_interactions(self, chapter_idx: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        with self.graph_manager.driver.session() as session:
            return session.execute_read(self._get_character_interactions_tx, chapter_idx)
    
    def find_similar_entities(self, entity_name: str, similarity_threshold: float = 0.8) -> List[Dict[str, Any]]:
        """
        ...
        """
        with self.graph_manager.driver.session() as session:
            return session.execute_read(self._find_similar_entities_tx, entity_name, similarity_threshold)
    ...
    def get_temporal_relationships(self, temporal_type: str) -> List[Dict[str, Any]]:
        ...
        """
        with self.graph_manager.driver.session() as session:
            return session.execute_read(self._get_temporal_relationships_tx, temporal_type)
    ...
    @staticmethod
    def _find_connected_entities_tx(tx, entity_name: str, max_depth: int) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _find_entities_by_relationship_tx(tx, predicate: str, chapter_idx: Optional[int]) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _get_character_interactions_tx(tx, chapter_idx: Optional[int]) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _find_similar_entities_tx(tx, entity_name: str, similarity_threshold: float) -> List[Dict[str, Any]]:
        ...
        """
        result = tx.run(query, entity_name=entity_name.lower(), threshold=similarity_threshold)
        return [dict(record) for record in result]
